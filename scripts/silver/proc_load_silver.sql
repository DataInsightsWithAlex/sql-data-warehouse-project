...
Script Purpose:
	...
	- Uses the 'INSERT INTO' command to load the data from bronze tables to silver tables
Parameters:
	None.
...
BEGIN
	...
	BEGIN TRY
		...
		SELECT REPLACE(cid, '-', ''), -- Remove the dash (-) to be consistent with the silver.crm_cust_info cst_key
				CASE
					WHEN UPPER(TRIM(cntry)) IN ('DE', 'GERMANY', 'DEUTSCHLAND') THEN 'Germany'
